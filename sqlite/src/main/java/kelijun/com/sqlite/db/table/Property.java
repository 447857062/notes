package kelijun.com.sqlite.db.table;

import java.lang.reflect.Field;
import java.lang.reflect.Method;
import java.util.Date;

import kelijun.com.sqlite.utils.FieldUtils;
        ...
        if(set!=null && value!=null){
            try {
                if (dataType == String.class) {
                    ...
                    set.invoke(receiver, Float.parseFloat(value.toString()));
                } else if (dataType == double.class || dataType == Double.class) {
                    set.invoke(receiver, Double.parseDouble(value.toString()));
                } else if (dataType == long.class || dataType == Long.class) {
                    set.invoke(receiver, Long.parseLong(value.toString()));
                } else if (dataType == Date.class || dataType == java.sql.Date.class) {
                    set.invoke(receiver, FieldUtils.stringToDateTime(value.toString()));
                } else if (dataType == boolean.class || dataType == Boolean.class) {
                    set.invoke(receiver, "1".equals(value.toString()));
                } else {
